...
static QString QVariant_to_string(const QVariant &v)
{
    ...
    // FIXME: what encoding is QVariant.typeName() really?
    const QString types = QLatin1String(typen);
    const QString ret =  types + QLatin1Char(':') + s;
    return ret;
}

static char const *QDbusMessageType_2_desc_latin1lit(const QDBusMessage::MessageType &t)
{
    switch(t) {
...
static QString QDBusMessage_argdesc(const QDBusMessage &m)
{
    ...
    const QString args_desc = argss.join(QStringLiteral(", "));
    return args_desc;
}

static QString QDBusMessage_desc(const QDBusMessage &m)
{
    const QString types = QLatin1String(QDbusMessageType_2_desc_latin1lit(m.type()));
    ...
    const QString errdesc = m.errorMessage().trimmed();
    ...
    if(!ret.isEmpty() && !service.isEmpty()) {
        ret += QLatin1String(" ");
    }
...
static bool is_bad_error(const QDBusMessage &m)
{
    ...
    switch(t) {
        ...
        case QDBusMessage::ErrorMessage: {
            ...
            else if(errname == QLatin1String("org.freedesktop.DBus.Error.ServiceUnknown")) {
                return false;
            }
            else if(errname == QLatin1String("org.freedesktop.DBus.Error.UnknownMethod")) {
                return false;
            }
            else {
                return true;
